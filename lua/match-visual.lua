 ...
 local default_config = {match_id = 118, hl_group = "Visual"} local highlight_group = "VisualMatch"


 nvim_set_hl(0, highlight_group, {default = true, link = default_config.hl_group})

 local default_match_id = default_config.match_id
 local visual_matches = {}

 local function set_default_match_id(id) _G.assert((nil ~= id), "Missing argument id on fennel/src/match-visual.fnl:22")
 default_match_id = id return nil end

 local function lines__3ematch_string(lines) _G.assert((nil ~= lines), "Missing argument lines on fennel/src/match-visual.fnl:25")
 ...
 local function add_matches(group, line_wise_3f, _6_, _8_) local _arg_7_ = _6_ local start_row = _arg_7_[1] local start_col = _arg_7_[2] local _arg_9_ = _8_ local end_row = _arg_9_[1] local end_col = _arg_9_[2] _G.assert((nil ~= end_col), "Missing argument end-col on fennel/src/match-visual.fnl:31") _G.assert((nil ~= end_row), "Missing argument end-row on fennel/src/match-visual.fnl:31") _G.assert((nil ~= start_col), "Missing argument start-col on fennel/src/match-visual.fnl:31") _G.assert((nil ~= start_row), "Missing argument start-row on fennel/src/match-visual.fnl:31") _G.assert((nil ~= line_wise_3f), "Missing argument line-wise? on fennel/src/match-visual.fnl:31") _G.assert((nil ~= group), "Missing argument group on fennel/src/match-visual.fnl:31")
 ...
 local match_string = lines__3ematch_string(lines)
 ...
 do local match_id = matchadd(group, match_string, 10, default_match_id, {window = win_id})
 ...
 local function order_positions(_16_, _18_) local _arg_17_ = _16_ local start_row = _arg_17_[1] local start_col = _arg_17_[2] local _arg_19_ = _18_ local end_row = _arg_19_[1] local end_col = _arg_19_[2] _G.assert((nil ~= end_col), "Missing argument end-col on fennel/src/match-visual.fnl:49") _G.assert((nil ~= end_row), "Missing argument end-row on fennel/src/match-visual.fnl:49") _G.assert((nil ~= start_col), "Missing argument start-col on fennel/src/match-visual.fnl:49") _G.assert((nil ~= start_row), "Missing argument start-row on fennel/src/match-visual.fnl:49")
 if ((start_row > end_row) or ((start_row == end_row) and (start_col > end_col))) then

 return {end_row, end_col}, {start_row, start_col} else
 return {start_row, start_col}, {end_row, end_col} end end

 local function get_range(line_wise_3f) _G.assert((nil ~= line_wise_3f), "Missing argument line-wise? on fennel/src/match-visual.fnl:55")
 ...
 local matches = add_matches(highlight_group, line_wise_3f, get_range(line_wise_3f))

 visual_matches = (matches or {}) return nil else return nil end end

 local function setup(_3fuser_config)
 local user_config = (_3fuser_config or {})
 if user_config.match_id then
 set_default_match_id(user_config.match_id) else end
 if user_config.hl_group then
 return nvim_set_hl(0, highlight_group, {link = user_config.hl_group}) else return nil end end

 return {setup = setup, set_default_match_id = set_default_match_id, match_visual = match_visual, remove_visual_selection = remove_visual_selection}
